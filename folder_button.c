...
static void update_arrow (FolderButton *self) {
    if (self->expanded) {
        gtk_image_set_from_icon_name (GTK_IMAGE (self->arrow), 
                                      "pan-down-symbolic");
    } else {
        gtk_image_set_from_icon_name (GTK_IMAGE (self->arrow), 
                                      "pan-end-symbolic");
...
static void folder_button_init (FolderButton *self) {
    ...
    // Header button with arrow and label
    ...
    self->arrow = gtk_image_new_from_icon_name ("pan-end-symbolic");
    gtk_box_append (GTK_BOX (header_box), self->arrow);

    self->label = gtk_label_new ("Folder");
    gtk_label_set_xalign (GTK_LABEL (self->label), 0.0);
    gtk_widget_set_hexpand (self->label, TRUE);
    gtk_box_append (GTK_BOX (header_box), self->label);

    self->button = gtk_button_new ();
    gtk_button_set_has_frame (GTK_BUTTON (self->button), FALSE);
    gtk_button_set_child (GTK_BUTTON (self->button), header_box);
...
static void folder_button_get_property (GObject    *object,
                                        guint       prop_id,
                                        GValue     *value,
                                        GParamSpec *pspec) {
...
static void folder_button_set_property (GObject      *object,
                                        guint         prop_id,
                                        const GValue *value,
                                        GParamSpec   *pspec) {
    FolderButton *self = FOLDER_BUTTON (object);
